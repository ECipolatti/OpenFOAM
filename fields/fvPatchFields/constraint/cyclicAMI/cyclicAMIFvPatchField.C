...
//    return tpnf;
  ...
  //__________________HECHO POR SANTIAGO_________
    Info<<"patchNeighbourFIELD MODIFICADO"<<endl;
    ...
    Type value = sum(pnf)/pnf.size();
    Info<<"El valor de value es: "<< value <<endl;

    forAll(result, faceI)
    {
        result[faceI] = value; //sum(pnf)/pnf.size();
    }
